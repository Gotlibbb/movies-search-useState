...
export const MovieComponent = React.memo((props: MoviePropsType) => {
    ...
    useEffect(() => {
        ...
        [props, init, url.filmNameUrl, url.movieId]
    )

    if (props.preloader) return <Preloader/>

    console.log("MovieComponent render")

    return <div className={style.moviePage}>
        <NavLink to={"/movies-search-useState/search-results/" + url.filmNameUrl + `/` + props.currentPage}>⇦ Back to
            search</NavLink>
